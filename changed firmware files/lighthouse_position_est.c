...
#include "controller_ae483.h" // AE 483 Final project addition 
...
// ADDED FOR AE 483 FINAL PROJECT 
static sweepAngleMeasurement_t sweepInfo;
...
static void estimatePositionCrossingBeams(const pulseProcessor_t *state, pulseProcessorResult_t* angles, int baseStation) {
  ...
  if (sensorsUsed == PULSE_PROCESSOR_N_SENSORS) {
    ...
    // AE 483 PROJECT ADDITION
    ae483UpdateWithLH(positionLog[0], positionLog[1], positionLog[2]);
...
static void estimatePositionSweepsLh2(const pulseProcessor_t* appState, pulseProcessorResult_t* angles, int baseStation) {
  const lighthouseCalibration_t* bsCalib = &appState->bsCalibration[baseStation];
  //sweepAngleMeasurement_t sweepInfo; COMMENTED OUT BECAUSE this is a static variable now
